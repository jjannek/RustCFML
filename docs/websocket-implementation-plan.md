...
**Authorization (P-auth)** shipped (v0.302.0). `secured=` handler annotation + `canJoin` room-join hook. Identity comes from `socket.data` (the channel populates it in onConnect from the session).
- `secured`: in `dispatch_ws_event_inner`, after resolving the handler, `ws_secured_annotation(struct, fname)` reads `__funcmeta_<name>.secured`; `ws_secured_ok(secured, socket.data)` authorizes — bare/`"true"` → `data.authenticated` truthy; `"a,b"` → `data.roles[]`/`data.role` intersect; `"false"` → opt out. Failure returns an Err → driver routes to onError (onConnect denial → handshake reject).
- `canJoin`: VM stashes the channel instance in `current_ws_instance` for the duration of the handler call; in `call_member_function`, before a `Socket.join` native dispatch, if the channel defines `canJoin` it's called (this-bound, recursion-guarded by taking the stash); a falsey/throw return makes `socket.join` throw `join to room [x] denied by canJoin`.
- **Core bugfix (prerequisite):** `socket.data.X = v` writes were a silent no-op + clobbered the `socket` local. Root cause: (1) the `GetProperty` opcode and `lookup_property` never consulted `CfmlNative::get_property` for a NativeObject (read returned Null); (2) `store_runtime_path` (the `SetDynamicVar` auto-vivify path that `obj.a.b=v` compiles to) replaced a non-Struct root with a fresh struct, overwriting the live `socket` handle. Fixed all three to be NativeObject-aware: reads consult `get_property`; the path-store descends into the reference-typed property (`socket.data` → shared struct) and mutates in place without overwriting the native local (`socket.data=v` routes to `set_property`). This makes the documented Phase-1 live-`socket.data` feature actually work.
- Tests: `secured_annotation_gates_handlers` + `canjoin_gates_room_joins` (websocket_raw.rs, now 9 live tests) + `auth.cfc` fixture (uses `socket.data` writes in onConnect — also the regression guard for the core fix). Docs: `docs/websockets.md` "Authorization" section + roadmap.
...
- **Auth** — ✅ DONE (see "Authorization (P-auth)" below). `secured=` annotation + `canJoin` hook.
